import type { GraveyardVerdict, OpkEvent, HealthEntity } from "./types";
...
export async function fetchHealth(): Promise<HealthEntity[]> {
  const res = await fetch("/api/opk/health", {
    method: "GET",
    cache: "no-store",
  });
